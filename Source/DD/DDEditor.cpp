...
void MetallicDDEditor::buildFactory()
{
...
juce::File MetallicDDEditor::presetsFile()
{
    auto dir = juce::File::getSpecialLocation(juce::File::userApplicationDataDirectory)
                   .getChildFile("MetallicDigitalDelay");
    dir.createDirectory();
    return dir.getChildFile("UserPresets.xml");
}

void MetallicDDEditor::refreshPresetBox()
{
...
void MetallicDDEditor::loadPreset(const DDPreset& p)
{
    ...
    setF("predelay", p.predelay); setF("size",      p.size);
    setF("density",  p.density);  setF("spread",    p.spread);
    setF("highcut",  p.highcut);  setF("lowfreq",   p.lowfreq);
    setF("metal",    p.metal);    setF("resonance", p.resonance);
    setF("attack",   p.attack);   setF("hold",      p.hold);
    setF("decay",    p.decay);    setF("dry",       p.dry);
    setF("wet",      p.wet);      setI("division",  p.division);
    setB("gate_on",  p.gateOn);   setB("reset_on_beat", p.resetOnBeat);
}

void MetallicDDEditor::applyByIndex(int id)
{
    int idx = id - 1;
...
void MetallicDDEditor::savePreset()
{
    auto* dlg = new juce::AlertWindow("Save Preset", "Enter a name:", juce::AlertWindow::NoIcon);
    dlg->addTextEditor("name","My Preset","Name:");
    dlg->addButton("Save",1); dlg->addButton("Cancel",0);
    juce::Component::SafePointer<MetallicDDEditor> safeThis(this);
    dlg->enterModalState(true,
        juce::ModalCallbackFunction::create([safeThis,dlg](int r)
        {
            if (r == 1 && safeThis != nullptr)
            {
                ...
                p.dry=g("dry"); p.wet=g("wet"); p.division=(int)g("division");
                p.gateOn=g("gate_on")>0.5f; p.resetOnBeat=g("reset_on_beat")>0.5f;
...
void MetallicDDEditor::deletePreset()
{
...
MetallicDDEditor::MetallicDDEditor(MetallicDDProcessor& p)
...
MetallicDDEditor::~MetallicDDEditor() { stopTimer(); }

void MetallicDDEditor::knob(juce::Slider& s, juce::Label& l,
...
void MetallicDDEditor::fader(juce::Slider& s, juce::Label& l,
                              const juce::String& txt, juce::Colour col)
{
...
void MetallicDDEditor::styleButton(juce::TextButton& b, juce::Colour col)
{
...
void MetallicDDEditor::timerCallback()
{
    meterLevel = proc.currentGateLevel.load();
    repaint();
}

void MetallicDDEditor::paint(juce::Graphics& g)
{
    g.fillAll(C_BG);
    ...
    g.setColour(C_PURP);
    g.drawText("METALLIC  DIGITAL  DELAY", 0, 6, getWidth(), 24, juce::Justification::centred);
...
juce::AudioProcessorEditor* MetallicDDProcessor::createEditor()
{
    return new MetallicDDEditor(*this);
}
